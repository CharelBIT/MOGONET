...
def train_epoch(data_list, adj_list, label, one_hot_label, sample_weight, model_dict, optim_dict, train_VCDN=True):
    ...
    if train_VCDN and num_view >= 2:
        ...
        # c = c[tr_idx, :]
        # prob_tr = F.softmax(c, dim=1).data.cpu().numpy()
        # print("\ntrain...", c_loss)
    return loss_dict
...
def train_test(data_folder, view_list, num_class,
    ...
    for epoch in range(num_epoch+1):
        ...
        if epoch % 1 == 0:
            ...
            print()
            assert len(trte_idx["te"]) == te_prob.shape[0]
            train_result_df = pd.DataFrame()
            for i, index in enumerate(trte_idx["te"]):
                train_result_df.loc[index, 'prob'] = te_prob[i, 1]
            tr_prob = test_epoch(data_trte_list, adj_te_list, trte_idx["tr"], model_dict)
            ...
            if num_class == 2:
                ...
                print("Train ACC: {:.3f}".format(accuracy_score(labels_trte[trte_idx["tr"]], tr_prob.argmax(1))))
                print("Train F1 weighted: {:.3f}".format(
                    f1_score(labels_trte[trte_idx["tr"]], te_prob.argmax(1), average='weighted')))
                print("Train F1 macro: {:.3f}".format(
                    f1_score(labels_trte[trte_idx["tr"]], te_prob.argmax(1), average='macro')))
            test_result_df = pd.DataFrame()
            for i, index in enumerate(trte_idx["tr"]):
                test_result_df.loc[index, 'prob'] = tr_prob[i, 1]
            train_result_df.to_csv(f'huwenjuan/result/train_result_{epoch}.csv')
            test_result_df.to_csv(f'huwenjuan/result/test_result_{epoch}.csv')
            print()